 ...
 */
export function useApollo<P, IP>(ac: ApolloClientParam) {
  return ({ ssr = false } = {}) =>
    (PageComponent: NextPage<P, IP>) => {
      const WithApollo = (
        pageProps: P & WithApolloOptions & { serverAccessToken: string },
      ) => {
        const serverAccessToken = pageProps.serverAccessToken;
      ...
      if (ssr || PageComponent.getInitialProps) {
        WithApollo.getInitialProps = async (ctx: ContextWithApolloOptions) => {
          let req;
          const { AppTree, ctx: { req: reqInContext, res } = {} } = ctx;
          if (!reqInContext) {
            req = ctx.req;
          }
          ...
          if (isServer()) {
            const cookies = req?.headers.cookie
              ...
              : undefined;
            console.log('useApollo log cookies', cookies);
            if (cookies?.jid) {
          ...
          const inAppContext = Boolean(ctx.ctx);
          const { apolloClient } = initOnContext(ac, ctx, serverAccessToken);
